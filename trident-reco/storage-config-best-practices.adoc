...
NetApp Trident admite la clonación de volúmenes al usar `ontap-nas` , `ontap-san` , y `solidfire-san` Controladores de almacenamiento. Al usar el `ontap-nas-flexgroup` o `ontap-nas-economy` controladores, la clonación no es compatible. La creación de un nuevo volumen a partir de un volumen existente dará como resultado la creación de una nueva instantánea.
