...
NOTE: Al utilizar una CR, cree manualmente el espacio de nombres de destino antes de aplicar la CR. Trident Protect crea espacios de nombres automáticamente solo cuando se usa la CLI.
...
*** *spec.storageClassMapping*: (_Opcional_) Asigna las clases de almacenamiento de origen a las de destino para PVC individuales dentro del mismo AMR. Cada entrada requiere un `source` y un nombre de clase de almacenamiento `destination`. Cuando se configura, `storageClassMapping` anula por completo `storageClassName`; los PVC que no tengan una entrada de origen coincidente usan la clase de almacenamiento predeterminada del clúster, no `storageClassName`.
+
Al usar  `storageClassMapping` en un CR de AppMirrorRelationship, ten en cuenta las siguientes restricciones:

+
**** `storageClassMapping` anula `storageClassName` por completo. Cuando ambos campos están configurados,  `storageClassName` se ignora.
**** Los PVC con clases de almacenamiento de origen que no tienen una entrada correspondiente en `storageClassMapping` usan la clase de almacenamiento predeterminada del clúster, no `storageClassName`.
**** Cada entrada requiere tanto un `source` como un `destination` valor.
**** Cambiar la clase de almacenamiento de destino puede provocar la eliminación y recreación de los PVC de destino existentes.
...
NOTE: Usa  `--storageclass-mapping` para asignar clases de almacenamiento de origen individuales a clases de almacenamiento de destino. Especifica uno o más pares  `source:destination` separados por comas (por ejemplo,  `"class-a:class-b,class-c:class-d"`). Usa  `*:destination-class` como comodín para asignar todos los PVC sin asignar a una sola clase de almacenamiento de destino. Cuando  `--storageclass-mapping` está configurado,  `--storage-class` se ignora.

+
*Ejemplo con una única clase de almacenamiento de destino (`--storage-class`):*
