...
* Para restaurar máquinas virtuales (VM) específicas de KubeVirt desde un archivo sin restaurar todos los recursos del espacio de nombres, usa el campo  `includedVirtualMachines` en un CR de restauración. Para CR compatibles, referencia de campos y ejemplos en YAML, consulta link:trident-protect-kubevirt-vms.html["Protege las máquinas virtuales de KubeVirt con Trident Protect"].
...
spec:
  ...
  appArchivePath: my-snapshot-path
----
+ . (_Opcional_) Si necesitas seleccionar solo determinados recursos de la aplicación para restaurarlos, añade un filtro que incluya o excluya los recursos marcados con etiquetas concretas:

+ NOTA: Trident Protect selecciona automáticamente algunos recursos debido a su relación con los recursos que tú seleccionas. Por ejemplo, si seleccionas un recurso de reclamación de volumen persistente y este tiene un pod asociado, Trident Protect también restaurará el pod asociado.

+ * *resourceFilter.resourceSelectionCriteria*: (Obligatorio para el filtrado) Utiliza `Include` o `Exclude` para incluir o excluir un recurso definido en resourceMatchers. Agrega los siguientes parámetros resourceMatchers para definir los recursos que se van a incluir o excluir: ** *resourceFilter.resourceMatchers*: Una matriz de objetos resourceMatcher. Si defines varios elementos en este array, se combinan como una operación OR, y los campos dentro de cada elemento (group, kind, version) se combinan como una operación AND. *** *resourceMatchers[].group*: (_Opcional_) Grupo del recurso que se va a filtrar. *** *resourceMatchers[].kind*: (_Opcional_) Tipo del recurso que se va a filtrar. *** *resourceMatchers[].version*: (_Opcional_) Versión del recurso que se va a filtrar. *** *resourceMatchers[].names*: (_Opcional_) Nombres en el campo metadata.name de Kubernetes del recurso que se va a filtrar. *** *resourceMatchers[].namespaces*: (_Opcional_) Namespaces en el campo metadata.name de Kubernetes del recurso que se va a filtrar. *** *resourceMatchers[].labelSelectors*: (_Opcional_) Cadena de selector de etiquetas en el campo metadata.name de Kubernetes del recurso, tal como se define en https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors["Documentación de Kubernetes"^]. Por ejemplo: `"trident.netapp.io/os=linux"`.
  ...
  resourceFilter:
    ...
    resourceMatchers:
      ...
      - group: my-resource-group-2
        ...
        labelSelectors: ["trident.netapp.io/os=linux"]
----
+ . Después de que completes el archivo  `trident-protect-snapshot-ipr-cr.yaml` con los valores correctos, aplica el CR:
