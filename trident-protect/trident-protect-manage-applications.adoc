...
NOTE: Para definir una aplicación con ámbito de máquina virtual (VM) que proteja máquinas virtuales (VM) específicas de KubeVirt por su nombre en lugar de espacios de nombres completos, usa el campo  `spec.includedVirtualMachines` en el Application CR o la bandera  `--virtual-machines` en la CLI. Para detalles, referencia de campos y ejemplos en YAML, consulta link:trident-protect-kubevirt-vms.html["Protege las máquinas virtuales de KubeVirt con Trident Protect"].
...
+ Para especificar la anotación cuando usas la CLI para crear una aplicación, puedes usar la bandera  `--annotation`.

+ * Crea la aplicación y utiliza la configuración global para el comportamiento de congelación del sistema de archivos:
...
----
+ * Crea la aplicación y configura la configuración local de la aplicación para el comportamiento de congelación del sistema de archivos:
...
----
+ * Puedes usar  `--resource-filter-include` y  `--resource-filter-exclude` para incluir o excluir recursos según  `resourceSelectionCriteria` como grupo, tipo, versión, etiquetas, nombres y namespaces, como se muestra en el siguiente ejemplo:
...
----
+ * Para definir una aplicación que utilice únicamente PVC, también debes incluir `PersistentVolume` y `VolumeSnapshotClass`` en el filtro de recursos. Las operaciones de instantáneas y copias de seguridad dependen de `PersistentVolume` (el volumen de ámbito de clúster vinculado a cada PVC) y `VolumeSnapshotClass` (el controlador de instantáneas), y fallarán si no se incluyen. Por ejemplo:
