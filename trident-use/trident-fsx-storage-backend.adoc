...
Los pasos de esta sección cubren cómo crear el backend por separado. Para obtener información sobre cómo utilizar la función Backend automático, consulta la sección xref:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap[Configura StorageClass y PVC] para más información.
...
Puede aprovisionar volúmenes SMB utilizando `ontap-nas` conductor. Antes de completar <<ONTAP SAN and NAS driver integration>> Complete estos pasos: link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["Prepárese para aprovisionar los volúmenes de SMB"].
