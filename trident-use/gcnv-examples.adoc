...
| `snapshotDir` | Acceso a la `.snapshot` directorio | `true`, `false` (el comportamiento por defecto puede variar. Establece explícitamente) "false" para NFSv3 
...
[%collapsible]
====
Al instalar Trident con una identidad en la nube de GKE, los backends se autentican como una identidad de carga de trabajo y omiten el bloque `apiKey`. La identidad en la nube se configura de la misma manera para ambos controladores de Google Cloud NetApp Volumes. La única diferencia es el valor de `storageDriverName`: usa `google-cloud-netapp-volumes` para volúmenes NAS (NFS o SMB) y `google-cloud-netapp-volumes-san` para volúmenes SAN (iSCSI block).


NOTE: La compatibilidad con identidades en la nube para el  `google-cloud-netapp-volumes-san` driver requiere Trident 26.06 o una versión posterior. Los volúmenes SAN se aprovisionan desde los grupos de almacenamiento Flex Unified, así que configura  `serviceLevel` en  `flex`.

.Controlador NAS (<code>google-cloud-netapp-volumes</code>)
[source, yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
...
----
.Controlador SAN (<code>google-cloud-netapp-volumes-san</code>)
[source, yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
