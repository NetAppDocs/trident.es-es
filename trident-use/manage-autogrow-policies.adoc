...
[role="lead"]
Después de crear las políticas de Autogrow, puedes verlas, actualizarlas y eliminarlas. También puedes supervisar qué volúmenes usan una política determinada.
...
IMPORTANT: Los cambios afectan a todos los volúmenes que utilizan la política. Cuando sea posible, prueba primero los cambios en un entorno que no sea de producción.
...
* *Efecto inmediato:* Todos los volúmenes que usan la política adoptan los nuevos parámetros en la próxima evaluación de crecimiento.
...
Las políticas de Autogrow usan la protección del finalizador para evitar el borrado accidental mientras los volúmenes las usan activamente.
...
----
. Si los volúmenes siguen utilizando la política, la eliminación pasa a un estado  `Deleting`. Comprueba qué volúmenes se ven afectados:
+
[source, console]
----
tridentctl get autogrowpolicy production-db-policy -o yaml
...
| Los volúmenes utilizan la política | La política entra `Deleting` en estado. Un finalizador bloquea la finalización hasta que se eliminan todos los volúmenes. 
...
Autogrow es compatible con los backends de ONTAP. Autogrow también es compatible con Google Cloud NetApp Volumes (GCNV) para volúmenes NAS y SAN.
...
* *Volúmenes existentes (implementaciones en entornos ya existentes):* Es posible que la función de autogrow no funcione con los volúmenes existentes, incluso si se aplica una Autogrow Policy válida. Esta limitación se debe a una migración en curso de publicaciones de volúmenes. Para confirmar que la migración se ha completado, revisa los registros del controlador Trident en busca de mensajes de  `"Migration completed"`.
...
Configura la anotación de PVC en  `"none"`. Esta configuración anula cualquier política a nivel de StorageClass para ese volumen:
...
Trident deja de ampliar el volumen. No genera más solicitudes de redimensionamiento para ese volumen, aunque el uso siga aumentando más allá de `usedThreshold`.

Por ejemplo, con esta política, Trident deja de ampliar el volumen una vez que alcanza los 100 GiB:
...
Sí. Cuando actualizas una política, todos los volúmenes que usan esa política adoptan los nuevos parámetros en la siguiente evaluación de crecimiento. No es necesario reiniciar ningún volumen.
...
Las políticas utilizan la protección de finalizadores. Si los volúmenes siguen utilizando la política, la eliminación pasa a un estado  `Deleting` y queda en espera hasta que elimines todos los volúmenes de la política.
...
kubectl annotate pvc <pvc-name> \
  trident.netapp.io/autogrowPolicy-
----
Después de que elimines todos los volúmenes, Trident libera el finalizador y elimina la política.



=== ¿Qué backends admite la ampliación automática?

Autogrow es compatible con los backends de ONTAP. También es compatible con Google Cloud NetApp Volumes (GCNV) tanto para volúmenes NAS como SAN.

Autogrow es compatible con los protocolos NFS, iSCSI, FCP y NVMe. Los volúmenes de bloques sin procesar NVMe requieren ONTAP 9.16.1 o una versión posterior.

Es posible que los volúmenes existentes en implementaciones en entornos brownfield requieran que se complete la migración de la publicación de volúmenes antes de que el autogrow surta efecto. Para comprobar el estado de la migración, consulta los registros del controlador Trident:
