...
NOTE: Si Trident ha aprovisionado volúmenes y snapshots a partir de este back-end que aún existen, al eliminar el back-end se evita que se aprovisionen nuevos volúmenes. El back-end seguirá existiendo en estado «Eliminado» y Trident seguirá gestionando esos volúmenes y snapshots hasta que se eliminen.
