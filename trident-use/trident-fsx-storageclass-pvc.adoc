...
El https://kubernetes.io/docs/concepts/storage/storage-classes/["Objeto de Kubernetes StorageClass"^] El objeto identifica a Trident como el aprovisionador que se utiliza para esa clase y le indica cómo aprovisionar un volumen.  Utilice este ejemplo para configurar Storageclass para volúmenes que utilizan NFS (consulte la sección Atributos de Trident a continuación para obtener la lista completa de atributos):
