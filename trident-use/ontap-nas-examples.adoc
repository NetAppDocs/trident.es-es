...
A partir de la versión 25.10, NetApp Trident es compatible conlink:https://docs.netapp.com/us-en/ontap-afx/index.html["Sistemas de almacenamiento NetApp AFX"^] .  Los sistemas de almacenamiento NetApp AFX se diferencian de otros sistemas basados ​​en ONTAP(ASA, AFF y FAS) en la implementación de su capa de almacenamiento.
...
En la configuración del backend de Trident , no es necesario especificar que su sistema sea un sistema de almacenamiento NetApp AFX.  Cuando selecciona `ontap-nas` como el `storageDriverName` Trident detecta automáticamente el sistema de almacenamiento AFX.  Algunos parámetros de configuración del backend no son aplicables a los sistemas de almacenamiento AFX, como se indica en la tabla siguiente.
