...
En `/deploy/operator.yaml`, actualiza `image: docker.io/netapp/trident-operator:26.02.0` para reflejar la ubicación de tu registro de imágenes. Tu link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Imágenes Trident y CSI"] puede estar en un registro o en registros diferentes, pero todas las imágenes CSI deben estar en el mismo registro. Por ejemplo:
